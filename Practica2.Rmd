...
Vamos a factorizar las variables categóricas. La variable ¨sex¨ se desconoce cómo está codificada, y las numéricas no categóricas vamos a transformarlas en variables numéricas:
...
Se detectan outliers las columnas:
-   **trtbps** : filas 9, 102, 111, 204, 224, 242, 249, 261, 267 
-   **chol** : filas 29, 86, 97, 221, 247 
-   **oldpeak** : filas 102, 205, 222, 251, 292

Se decide quitarlos. Ejemplo de limpieza de outliers:
```{r,echo=FALSE}
boxplot(heart$trtbps,main="Columna boxplot anterior a la limpieza")
```
...
for (columnName in numericas) {
  ...
  if (length(outliers_higher) | length(outliers_higher)){
    ...
    #    columnName,
    #    " Filas: ",
    #    outliers_lower,outliers_higher,"\n")
...
```{r,echo=FALSE}
boxplot(heart$trtbps,main="Columna boxplot posterior a la limpieza")
```
